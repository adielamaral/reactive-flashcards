...
public abstract class AbstractHandleException<T extends Exception> {
    ...
    abstract Mono<Void> handlerException(ServerWebExchange exchange, final T ex);

    protected Mono<Void> writeResponse(ServerWebExchange exchange, final ProblemResponse problemResponse) {
        return exchange.getResponse()
                .writeWith(Mono.fromCallable(() -> new DefaultDataBufferFactory().wrap(objectMapper.writeValueAsBytes(problemResponse))));
    }

    protected void prepareExchange(ServerWebExchange exchange, final HttpStatus statusCode) {
