...
import static br.com.devadiel.reactiveflashcards.domain.exception.BaseErrorMessage.*;
...
public class ApiExceptionHandler implements WebExceptionHandler {
    ...
    private Mono<Void> writeResponse(ServerWebExchange exchange, final ProblemResponse problemResponse) {
        return exchange.getResponse()
                .writeWith(Mono.fromCallable(() -> new DefaultDataBufferFactory().wrap(objectMapper.writeValueAsBytes(problemResponse))));
    }

    private void prepareExchange(ServerWebExchange exchange, final HttpStatus statusCode) {
